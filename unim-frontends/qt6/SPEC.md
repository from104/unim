...
5. 상태 필드 초기화 (focusObject, composing, popupActive, cursorRect)
6. `setAutoTypeFixCallback()` — `AutoTypefixApply` 시그널 구독.
...
7. `setCommitTextCallback()` — `CommitText` 시그널 구독 (standalone 팝업의 마우스 클릭 커밋 수신용)
8. `setShowEmojiPopupCallback()` — `ShowEmojiPopupV2` 시그널 구독 (`m_popupActive = true` 마킹)
9. `setHidePopupCallback()` — `HidePopup` 시그널 구독 (`m_popupActive = false`)
...
플러그인은 트리거와 `m_popupActive` 플래그 관리만 담당합니다. (동작은 Qt5와 동일합니다.)
...
> [!NOTE]
> 동작 로직 (키 처리, 포커스 관리, Preedit, 팝업 위임)은 **Qt5와 완전히 동일**합니다.
...
| 모듈명 | 컴포넌트 |
|--------|---------|
| `QT6_IM` | `input_context.cpp` (키 처리, 포커스, preedit, m_popupActive) |
