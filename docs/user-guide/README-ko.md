...
> **북마크**: 자주 쓰는 한자를 즐겨찾기에 등록할 수 있다. 후보에 포커스를 둔 상태에서 **Space** 키 → ☆(미등록) ↔ ★(등록) 토글. 등록된 한자는 `HanjaBookmarkChanged` DBus 시그널로 모든 열린 팝업이 즉시 갱신된다.
...
#### prefix-avoidance (오작동 방지)

`wood` 입력 후 `y`를 추가로 칠 가능성이 있는 동안은 교정을 보류한다. `wood ` (스페이스)가 들어오는 순간 비로소 확정. 즉 `woody`를 치는 사람이 `wood`로 잘못 교정될 일은 없다.
