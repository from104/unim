...
> **0.2.0 갱신 주석**: 처음 두 항목(*낱자 provenance 태깅*, *문맥 의존 키 해석*)은 v2에서 구현됨. `key_meta.vowel_combine_head` + `key_meta.context_alt` 데이터화로 엔진 재설계 없이 해결. 자세한 내용은 [LAYOUT_PROFILE_V2.md](LAYOUT_PROFILE_V2.md) 참고. 나머지 항목은 여전히 별도 단계.

- ~~**낱자 provenance 태깅**~~ → **v2 구현 (`vowel_combine_head`)**: 같은 ㅗ/ㅜ라도 "어느 키에서 왔는지"를 composer 큐 평행 meta(`JamoMeta`)로 추적. enum 전면 개편 없이 boolean 한 필드로 표현. ko_3bul390/391의 `9`·`/` ↔ `v`·`b` 이중모음 결합 분리 달성.
- ~~**문맥 의존 키 해석**~~ → **v2 구현 (`context_alt` + 9개 `ContextCondition`)**: `/`가 초성-only일 때만 ㅗ, 그 외 리터럴로. press_key가 keyboard_map 조회 직전 분기. composer 상태를 9개 helper로 평가.
...
**결과(v1 시점)**: 세벌식 390 원본 규약의 일부는 v1에서 재현되지 않는다. 구체적으로 `9`-ㅜ/`/`-ㅗ가 이중모음 첫 모음이 아닐 때도 커밋되고, `/`는 초성 없이 눌러도 ㅗ로 나간다. 원본 충실 재현은 엔진 재설계 이후로 연기.

**결과(v2 시점, 0.2.0+)**: 위 두 케이스 모두 v2에서 정확 재현. v/b 키 ㅗ·ㅜ는 후속 모음과 결합하지 않고 음절 분리되며, `/`는 초성-only 상태에서만 ㅗ로 동작 — 그 외엔 리터럴 `/`. v2 구현은 [LAYOUT_PROFILE_V2.md](LAYOUT_PROFILE_V2.md) §3·§4 참고.
