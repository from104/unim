...
"대한민국"처럼 여러 음절로 된 단어도 한 번에 한자로 바꿀 수 있다.

1. 단어를 끝까지 입력한다 (예: `대한민국` — 마지막 글자 "국"이 아직 조합 중이어야 한다).
2. **한자 키**를 누른다. 사전에 있는 단어면 단어 전체가 후보로 뜬다 (`大韓民國` 등).
3. 숫자 또는 화살표+Enter로 확정한다.

> **조사를 붙이기 전에 눌러야 한다.** "대한민국은"처럼 조사를 이미 붙였다면 한자 키는 그 순간 조합 중인 꼬리 부분(사전에 있으면 "국은"처럼 여러 글자, 없으면 마지막 음절)만 바꾼다. 단어를 다 쓴 다음, 조사를 붙이기 **전**에 한자 키를 누른다.
>
> **팝업이 뜬 채로 한자 키를 다시 누르면 대상이 앞 글자부터 줄어 사전에 있는 더 짧은 단어로 바뀐다.** 예: `대한민국` → `민국` → `국`.
>
> 마우스로 커서를 다른 자리로 옮긴 뒤 바로 이어서 타이핑하면, 이미 입력해 둔 다른 단어가 헤더에 잘못 뜰 수 있다. 이때는 **Esc**로 팝업만 닫으면 된다 — 이미 입력된 글자는 건드리지 않는다.

#### 선택한 단어 한자 변환

이미 입력해 둔 한글 단어를 선택(마우스·Shift+방향키)해서도 한자로 바꿀 수 있다.

1. 앱에서 한글 단어를 선택한다(마우스 또는 Shift+방향키).
2. **한자 키**를 누른다.
3. 숫자 또는 화살표+Enter로 확정 — 선택 영역이 그대로 한자로 바뀐다.
...
| GNOME Shell 확장 | ✔ |
| 순수 Wayland | ✔ |
| GTK3 | ✘ (방금 친 단어 변환은 가능, 선택 시 이모지 팝업) |
| XIM | ✘ (방금 친 단어 변환은 가능, 선택 시 이모지 팝업) |
<!-- @endplatform -->

선택한 문자열이 사전에 없거나 한글이 아니면 평소처럼 **이모지 팝업**이 뜬다 — 이모지를 확정하면 선택이 그 이모지로 바뀐다(**Esc**를 누르면 선택은 그대로 남는다).

#### 한자 출력 형식

확정할 때 들어가는 문자열 형식을 바꿀 수 있다 — [설정] › [한자 출력 형식]에서 **漢字** / **한자(漢字)** / **漢字(한자)** 중 고른다. 단어 변환과 한 글자 변환 모두 같은 형식을 쓴다.

<!-- @platform:linux -->
CLI 로는 `unim-cli config set hanja-output-format hangul-hanja` 처럼 바꾼다.
<!-- @endplatform -->

#### 단어 즐겨찾기

단어 후보도 음절 후보와 똑같이 **Space** 키로 ☆/★를 토글해 즐겨찾기에 등록할 수 있다. 다만 즐겨찾기는 **같은 단어의 팝업에서만** 재사용된다 — "대한민국"에서 등록한 순서는 "대한민국" 팝업에만 적용되고 다른 단어 팝업에는 영향이 없다.
...
korean:
  ...
  hanja_output_format: Hanja  # Hanja / HangulHanja / HanjaHangul — 대문자로 시작
  word_mode_apps:
    - winword.exe
```
<!-- @endplatform -->
...
|  | 한자 출력 형식 | `漢字`/`한자(漢字)`/`漢字(한자)` — 한 글자 변환과 단어 변환에 모두 적용, [4.2](#42-한자-변환-hanja) 참고 |
...
>
> **이모지 입력**은 별도 스위치가 없다 — 한자 팝업과 같은 경로로 항상 켜져 있다. 조합 중이 아닌 상태에서 **한자 키(또는 F9)** 를 누르면 이모지 팝업이 뜬다(단, 사전에 있는 한글 단어를 선택한 상태라면 선택 단어 한자 변환이 우선한다 — [4.2](#42-한자-변환-hanja) 참고). 즉 같은 키가 상황에 따라 **한자 · 특수문자 · 이모지** 세 용도로 자동 전환된다.
<!-- @endplatform -->
...
- **저장 정책**: 빌트인 자판은 읽기 전용이라 「다른 이름으로 저장」만 가능. 사용자 자판은
  ...
  이 JSON 파일을 손으로 고친 경우에는 다음번 설정 저장(어떤 항목이든) 또는 재로그인 때 반영된다 —
  데몬이 자판 파일 자체를 감시하지는 않는다.
...
>
> **한 가지 예외 — 자판을 바꾸는 순간 글자를 조합 중이라면**: 새 자판은 **지금 조합 중인 글자가 끝난 뒤부터** 적용된다(음절 하나가 아니라 밑줄이 사라질 때까지 = 단어 단위 확정을 쓰면 단어 경계까지). 조합을 중간에 끊어 글자를 망가뜨리지 않으려는 의도된 동작이다. 바로 확인하고 싶으면 스페이스나 `Esc`로 조합을 끝낸 뒤 입력하면 된다.
<!-- @endplatform -->
