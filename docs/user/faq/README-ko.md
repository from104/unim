...
> UNIM 0.4.4에 대해 사람들이 정말 자주 묻는 질문 모음.
...
- 그때까지 무결성 확인은 **SHA256 대조**로 한다. 릴리스에 함께 올라오는 `SHA256SUMS-msi` 의 값과 비교하면 된다.
  ...
  ```powershell
  Get-FileHash .\unim-0.4.4-x64.msi -Algorithm SHA256
  ```
...
4. **스크립트 전문 공개** — [main 브랜치](https://github.com/from104/unim/blob/main/install.ps1)에 그대로 공개돼 있다. 실행 전에 받아서 읽어 볼 수 있고, 설치할 버전을 직접 고정할 수도 있다.
   ...
   # 특정 버전 고정
   $env:UNIM_VERSION='v0.4.4'; irm https://raw.githubusercontent.com/from104/unim/main/install.ps1 | iex
   ```
...
1. **키를 먼저 확인** — `한자` 키(또는 `F9`)는 상황에 따라 갈린다.
   - 한글을 조합한 상태(방금 친 한 글자 또는 단어) → **한자** 후보
   - 이미 입력해 둔 한글 단어(사전에 있는 것)를 마우스 또는 Shift+방향키로 선택한 상태 → 그 단어 **한자** 후보
   - 초성 하나만(예 `ㅁ`) 조합한 상태 → **특수문자**
   - 아무것도 조합하지 않고 선택도 없는 상태 → **이모지**
