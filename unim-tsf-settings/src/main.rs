...
//
// 감지/설정 헬퍼는 windows 전용 크레이트(#![cfg(windows)])에 있으므로 cfg 로 감싼다.
// 비Windows(크로스 체크/린트)에서는 무해한 폴백을 제공해 워크스페이스가 계속 컴파일된다.
#[cfg(windows)]
mod wiz {
    ...
    pub fn set_default_on_startup(v: bool) {
...
mod wiz {
    ...
    pub fn set_as_default() {}
    pub fn set_default_on_startup(_v: bool) {}
    pub fn is_korean_language_installed() -> bool {
        true
    }
    pub fn open_language_settings() {}
...
fn build_wizard_pages(mode: WizardMode, seen: Option<&str>) -> Vec<i32> {
    ...
    if is_new(WIZ_INTRODUCED) && !wiz::is_korean_language_installed() {
        pages.push(WIZ_LANGPACK);
    }
    if is_new(WIZ_INTRODUCED) && !wiz::is_default_ime() {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 인자가 없으면 이 블록은 통째로 건너뛰고 기존 일반 설정 모드로 동작(무회귀).
    // 진행 중에는 아무것도 저장하지 않고, [마침] 시에만 config 를 일괄 저장한다.
    if let Some(mode) = parse_wizard_mode() {
        let seen = wiz::get_wizard_seen_version();
        ...
        ui.on_wizard_open_langpack(|| wiz::open_language_settings());
        ...
        // + seen 버전 기록. '설정 열기' 체크면 그 자리에서 설정 모드로 전환, 아니면 닫기.
        {
            ...
            ui.on_wizard_finish(move || {
                ...
                if showed_default_ime && ui.get_wiz_default_ime_checked() {
                    wiz::set_as_default();
                    wiz::set_default_on_startup(true);
                }
                wiz::set_wizard_seen_version(env!("CARGO_PKG_VERSION"));
