...
### Changed
- **`EnglishLayout` enum removed (Phase 9)**: Symmetric to the Korean change. `english.layout` is now a String (built-ins: `qwerty`/`dvorak`/`colemak`/`colemak_dh`/`workman`). Legacy YAML values (`layout: Qwerty`) are auto-normalized via a serde `from = "EnglishConfigCompat"` layer. `deserialize_with` on `BlacklistEntry.english_layout` handles the matching migration in `typefix-blacklist.yaml`. DBus/CLI accept profile name strings directly with legacy names auto-promoted. C API (`unim-capi`): `UnimEnglishLayout` enum deleted; setters/getters now take/return C strings; `unim_english_layout_{name,display_name,at}` take `size_t` index. New helpers in `src/config.rs`: `normalize_english_layout_name`, `english_layout_keymap_name`, `english_layout_display_name`, `english_layout_top_row_labels`, `ENGLISH_LAYOUT_BUILTINS`. `keycode::to_char_for_layout` now takes `&str`.
