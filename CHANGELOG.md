...
- **Per-distribution builds are now checked for "does it work", not just "does it install".** The matrix used to stop at build, package count and an install smoke test; nothing ever checked that GTK and Qt actually load the installed IM modules — the very reason for per-distro rebuilds (Qt's private ABI) had never been proven by execution. Each of the six containers now verifies, right after installing, GTK3 cache registration, strict-binding loads of the GTK4/Qt5/Qt6 modules, and the daemon's D-Bus response and auto-activation, then drives real keystrokes into GTK3/GTK4/Qt5/Qt6/XIM apps on a virtual display (Xvfb) through thirteen scenarios — 2-beolsik composition, commit, backspace, mode toggle and more. Ubuntu 24.04 gets the same typing check on every push. The RHEL 10 family skips only the typing check, since Xvfb is not packaged there. On Windows the MSI is actually installed on the runner and checked for files, registry registration, DLL loading, Hangul typing in Notepad, and clean removal (warning-only for now, as a first rollout).
- **Removing the reasons antivirus mistakes the Windows build for a trojan.** On 2026-09-03 Windows Defender quarantined `unim_tsf.dll` under an ML heuristic (`Trojan:Win32/Bearfoos.B!ml`), killing the IME. Inspecting the binary showed no hooking or injection; the cause was distribution hygiene — an unsigned 8 MB keyboard DLL with every file-info field (company, product, version) blank is suspicious on its own. Three changes: ① every DLL and EXE shipped in the MSI now gets version information stamped at build time; ② CI scans the artifacts with Defender first, so a false positive is caught before users hit it; ③ a SignPath Foundation (free code signing for open source) pipeline is wired up to sign the MSI and the files inside it — signed builds start with the first release after the foundation approves the project and the secrets are registered. Recovery steps for anyone already hit are in troubleshooting 4-W.
...
- **Saving settings no longer loses the syllable being composed or breaks the composition.** The new functional typing check caught this. When the config file changes, the daemon rebuilds the Hangul composer; that pushed the in-progress syllable into the commit buffer, but the next key never flushed that buffer, so the character vanished for good — and because the composer had been swapped, the following Backspace misbehaved too. Leftover commit text is now always carried on the next response, and the composer is rebuilt only when a Hangul-layout-related setting actually changed, and only after the current composition ends. Switching layouts finishes the word you are typing on the old layout and applies the new one from the next word.
- **The XIM server no longer dies when an app closes, which used to block XIM input in every app opened afterwards.** Handling the caret-position update an app sends as it closes, the server queried a window that was already gone, and the resulting X error terminated the process; every app started after that failed to connect to the IM server. The error is now ignored for that one query and the position report is skipped.
