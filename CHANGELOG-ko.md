...
- **4개 GTK 앱 고유 아이콘**: 인디케이터·설정·키맵 스튜디오·타자 연습 앱이 각각 고유 아이콘을 가집니다. 앱 ID가 역DNS 명명으로 통일되어(앱 ID == `.desktop` 파일명 == 아이콘 이름, 예: `io.github.from104.unim.Settings`), GNOME Wayland 작업표시줄·오버뷰에 아이콘이 정상 표시됩니다. 트레이 한/영 상태 아이콘은 그대로 유지됩니다.

### 🔄 달라진 점

- **키맵 스튜디오 재설계**: 기존 좌측 사이드바 + 2탭 구성이 헤더 3단 드롭다운(언어 › 출처 › 자판) + 4탭(기본 / 자판 / 조합 / 확장) 구성으로 전면 개편되었습니다. 빌트인 자판은 보호되어("다른 이름으로 저장"만 가능), 사용자 자판은 제자리에 저장됩니다. `unim-keymap-studio/README.md` 참고.
...
- **키맵 도구 키보드 위젯 공유**: 키맵 스튜디오와 타자 연습이 5행 키보드 위젯 하나를 공유하도록 정리하고(중복 3벌 → 1벌), 죽은 코드(구식 `KeyboardWidget`·`ProfileSidebar` 등)를 제거했습니다.
