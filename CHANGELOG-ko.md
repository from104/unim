...
고정키 사용자를 위한 수정과, 페도라 계열에 빠져 있던 rpm 패키지를 채우는 판입니다.

### 고쳐짐

- **rpm 패키지가 릴리스에 올라오지 않던 문제**: 0.4.0 에는 deb 와 Windows MSI 만 있고 rpm 이 없어 페도라·RHEL 계열에서는 한 줄 설치가 아예 막혀 있었습니다. 빌드가 두 곳에서 깨져 있었습니다 — ① `rpmbuild` 가 `MAKEFLAGS` 에 넣는 `--` 구분자를 jemalloc 빌드 스크립트가 삼키다 실패했고, ② Qt 6.8 이상에서는 `Qt6::GuiPrivate` 를 명시적으로 요청해야 하는데 그러지 않았습니다(Qt 6.4 를 쓰는 deb 쪽은 무사해서 드러나지 않았습니다). 페도라 43 에서 실제 빌드까지 확인했습니다.

- **고정키(Sticky Keys)로 만든 조합이 가끔 듣지 않던 문제** (GNOME Wayland): 텍스트 입력란 안에서 `Ctrl`→`A`, `Alt`→`F` 같은 조합이 간헐적으로 수정자 없이 전달돼 그냥 글자가 입력됐습니다. 입력기가 키를 가로챈 뒤 되돌려 보내면 GNOME 이 고정키 래치를 다시 입히는 지점을 건너뛰고, 되돌아온 키가 한 박자 늦게 도착하는 사이 래치가 먼저 풀리는 것이 원인이었습니다. 이제 입력기가 소비하지 않을 키는 가로채지 않고 그대로 넘겨 래치가 살아 있습니다. `Shift`+`Home`·`Shift`+방향키 같은 선택 조작도 같은 결함이었고 함께 고쳐졌습니다. 한 번에 한 키만 누를 수 있는 분들에게는 조합키를 쓸 수 있느냐 없느냐의 문제입니다.
