...
> 2026-07-19 에 붙었던 이전 v0.4.0 태그·릴리스는 회수됐습니다 — 태깅 직후 `guids.wxi` 버전 불일치로 windows-msi 릴리스 게이트가 깨져 있던 것이 확인되어(`65c66f8` 에서 수정) 릴리스를 되돌리고 다시 진행했습니다. 이 항목이 유효한 v0.4.0 입니다.
...
- **Wayland 프런트엔드: 네이티브 Wayland 컴포지터(Sway, 단독 Hyprland 등)에서 전 키가 오독되던 문제**: 순수 Wayland 프런트엔드가 데몬에 X11 방식 키코드(raw evdev + 8)를 보냈는데, 데몬은 raw evdev 를 기대합니다 — 그 결과 `Shift+F9` 뿐 아니라 사실상 모든 키 조회가 8 만큼 어긋났습니다. GTK 프런트엔드가 이미 보내는 대로 raw evdev 를 보내도록 정정했습니다. GNOME 세션은 확장 경로를 쓰므로 영향이 없고, `unim-wayland` 는 그동안 Mutter 하에서 즉시 종료돼 이 경로가 실측되지 않았습니다.

- **GNOME Wayland(Chrome·GTK3/4·네이티브 Wayland 앱)와 포커스 유지 중 필드 성격 변경 시 비밀번호 필드 억제 수정**: GNOME 확장의 content-purpose 처리가 빈 스텁이어서, GTK3/4·네이티브 Wayland Chrome 이 전부 이 경로로 수렴하는 GNOME Wayland 에서는 민감 필드 억제가 조용히 동작하지 않았습니다. 이제 실제로 배선되었고(비밀번호 유형을 알리지 않는 앱을 위한 hidden-text 힌트 폴백 포함), GTK3/4 도 자체 판정을 추가해 Epiphany/WebKitGTK 등을 함께 커버하며, 두 경로 모두 포커스가 유지된 채 필드 성격이 바뀌는 경우(예: "비밀번호 표시" 토글)까지 추적하고 focus-out 시 한글 모드로 정확히 복귀합니다.

### 🪟 Windows 지원 (실험적)

이번 개발 주기에 Windows 포팅이 크게 진행되었습니다. 실기기 검증이 진행 중인 실험적 지원으로, 아래는 구현된 범위입니다.

- **TSF 완전 네이티브 아키텍처**: 언어바·조합/후보 팝업(한자·특수문자·이모지 — 9×9 격자·즐겨찾기·페이지 이동)·AutoTypeFix UI(순방향·역방향·수동·억제 목록·실행취소)가 이제 단일 `unim_tsf.dll` 에 통합되어, 종전의 별도 도우미 실행 파일(`unim-windows`)을 대체합니다. MSI 는 여전히 별도 목적의 소형 실행 파일 두 개 — `unim-settings.exe`(크로스플랫폼 Slint 설정앱/첫 실행 마법사), `unim-popup-win.exe`(한자·특수문자·이모지 팝업을 그리는 별도 프로세스 렌더러) — 를 함께 배포하지만, `unim-windows` 처럼 일상적인 TSF 동작이 별도 도우미에 의존하지는 않습니다. 공유 코어·리눅스 프런트엔드는 수정하지 않았습니다.

- **콘솔·IMM32 앱 한글 조합 (CUAS 계약 준수)**: WezTerm·텔레그램 등 인라인 조합을 쓰는 콘솔/IMM32 앱에서 한글 조합이 복구되었습니다.

- **32비트 앱 지원 (카카오톡·한컴 등)**: 32비트 앱이 64비트 전용 TIP를 찾지 못하던 문제를 해결하기 위해 32비트 TSF TIP(`unim_tsf32.dll`)를 함께 등록합니다. Win11에서 무의미한 IMM32 `.ime` 등록은 폐기했습니다. (`unim-imm32` 크레이트 자체는 진단·연구용 빌드로 소스 트리에 남아 있으나 MSI 에는 포함되지 않습니다.)

- **IMM32 앱(카카오톡·한컴 등)에서 AutoTypeFix 토글 단축키가 동작하도록 수정**: TSF 에는 있던 토글 드레인이 IMM32 에는 없어서, 토글 핫키를 눌러도 키 매칭만 될 뿐 저장된 설정의 `auto_typefix` 값이 실제로 바뀌지 않는 조용한 no-op 이었습니다. 이제 정상 반영됩니다. (TSF 가 갖춘 비프·설정 파일 실시간 감지·언어바 반영은 IMM32 에 해당 인프라가 없어 이식되지 않았습니다.)
...
- **MSI 배포**: windows-rs 0.62.2로 올리고 WiX 3.x 기반 MSI 빌드 체인을 정비했습니다. 참고: Windows MSI 는 더 오래 걸리는 별도 CI 워크플로에서 빌드되어, 리눅스 `.deb`/`.rpm` 패키지보다 몇 분 늦게 GitHub Release 에 첨부될 수 있습니다. 한 줄 설치 스크립트가 체크섬 매니페스트를 찾지 못한다고 하면 잠시 후 다시 시도하세요.
