...
# ── 5. 릴리스 자산 이름 정규화 ──────────────────────────────────────────────
# GitHub 은 릴리스 자산 이름에서 `~` 를 `.` 로 바꿔 저장한다. 파일은 `~` 인 채
# 올리고 매니페스트에도 `~` 로 적으면, install.sh 가 매니페스트의 이름으로
# 내려받다가 404 를 맞는다 (v0.4.2 실측 — deb 3배포판 전부 설치 불가).
# 그래서 **업로드 전에 파일 자체를 GitHub 이 쓸 이름으로 바꾼다.** 매니페스트를
# 추측으로 맞추는 대신 파일명·자산명·매니페스트를 한 이름으로 통일하는 쪽이
# GitHub 의 치환 규칙 변화에도 안전하다.
# 패키지 **내부** 버전(debian/changelog 의 `-1~ubuntu26.04`)은 그대로 둔다 —
# apt 의 버전 비교 규칙에서 `~` 가 의미를 갖기 때문이다. 파일명만 바꾼다.
for f in debs/*.deb; do
    base=$(basename "$f")
    safe=${base//\~/.}
    if [ "$base" != "$safe" ]; then
        mv -- "$f" "debs/${safe}"
        echo "  이름 정규화: $base → $safe"
    fi
done
