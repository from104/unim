...
use crate::popup::emoji::EmojiPopup;
use crate::popup::hanja::HanjaPopup;
use crate::popup::special::SpecialPopup;
                ...
                while let Ok(action) = rx.try_recv() {
                    match action {
                        ...
                        GuiAction::OpenSettings => {
                            // settings GUI는 별도 프로세스(unim-gui-gtk --settings)로 분리.
                            // popup-service는 spawn만 하고 settings 자체는 표시하지 않는다.
                            let _ = std::process::Command::new("unim-gui-gtk")
                                .arg("--settings")
                                .spawn();
...
// settings 다이얼로그는 unim-gui-gtk가 담당하므로 popup-service에서는 run_settings_only 제거.
...
fn load_css() {
    ...
    let css = format!(
        "{}\n{}\n{}\n{}",
        MODE_POPUP_CSS,
        crate::popup::hanja::popup_css(),
        crate::popup::special::popup_css(),
        crate::popup::emoji::popup_css()
    );
