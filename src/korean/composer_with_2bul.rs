...
/**
 * 두벌식 한국어 입력 방식의 조합 로직을 구현한 한국어 컴포저입니다.
 *
 * `BaseKoreanComposer`를 기반으로 두벌식 키보드 입력의 특징적인 동작,
 * 특히 '도깨비불 현상'과 중성 뒤 초성 입력 시 종성 변환 규칙을 처리합니다.
 *
 * `KoreanComposer` 트레이트를 구현하여 기본적인 한국어 조합 인터페이스를 제공합니다.
 */
#[derive(Debug, Default)]
pub struct KoreanComposer2Bul {
    /// 기본적인 한국어 조합 상태와 로직을 관리하는 내부 컴포저입니다.
    /// `KoreanComposer2Bul`은 두벌식 특화 로직을 추가하고,
    ...
    fn handle_dokkaebi_effect(&mut self, jamo: JamoEnum) -> Option<Option<char>> {
        ...
        if let Some(JamoEnum::Jong(jong)) = last_jamo {
            ...
            if matches!(jamo, JamoEnum::Jung(_)) {
                ...
                let current_char = self.force_compose_korean(); // Uses self, which is fine
...
impl KoreanComposer for KoreanComposer2Bul {
     ...
     * 이 맵은 `BaseKoreanComposer`에 의해 관리됩니다.
     */
    ...
    /**
     * 현재 조합 중인 한국어 음절(`KoreanChar`)에 대한 가변 참조를 반환합니다.
     * 이 객체는 `BaseKoreanComposer`에 의해 관리됩니다.
     */
     ...
     *
     * 위의 특수 규칙에 해당하지 않으면, `BaseKoreanComposer::add_jamo`를 호출하여
     ...
     * # 반환값
     *
     * * `Some(char)`: 자모 입력으로 인해 이전 음절 조합이 완료된 경우, 완성된 한국어 음절.
     *                 (주로 도깨비불 현상 발생 시 반환됨)
     ...
     * `BaseKoreanComposer::remove_jamo`에 작업을 위임합니다.
     *
     * # 반환값
     *
    ...
    /**
     * 현재 `jamo_queue`의 자모들을 조합하여 `current_korean` 상태를 업데이트합니다.
     *
     ...
     * 그 외의 경우는 `BaseKoreanComposer::compose_korean`에 작업을 위임합니다.
     *
     * # 반환값
     *
     ...
     * `BaseKoreanComposer::force_compose_korean`에 작업을 위임합니다.
     *
     * # 반환값
     *
    ...
    /**
     * 현재 한국어 조합이 진행 중인지 확인합니다.
     * `BaseKoreanComposer::is_compose`에 작업을 위임합니다.
     *
     * # 반환값
     *
    ...
    // --- 내부 조합 함수 위임 ---
    // 아래 함수들은 `BaseKoreanComposer`의 구현을 그대로 사용합니다.
    // 각 함수의 상세 설명은 `BaseKoreanComposer` 또는 `KoreanComposer` 트레이트 주석 참고.

    /**
     * 초성 조합 로직을 수행합니다. (내부 사용)
     * `BaseKoreanComposer::compose_cho`에 위임합니다.
     */
     ...
     * `BaseKoreanComposer::compose_jung`에 위임합니다.
     */
     ...
     * `BaseKoreanComposer::compose_jong`에 위임합니다.
     */
    ...
    /**
     * 현재 조합 중인 한국어 음절의 자모를 모두 지웁니다.
     * `BaseKoreanComposer::clear_jamo`에 위임합니다.
     */
    ...
    /**
     * 현재 조합된 초성을 반환합니다.
     * `BaseKoreanComposer::get_current_cho`에 위임합니다.
     */
    ...
    /**
     * 현재 조합된 중성을 반환합니다.
     * `BaseKoreanComposer::get_current_jung`에 위임합니다.
     */
    ...
    /**
     * 현재 조합된 종성을 반환합니다.
     * `BaseKoreanComposer::get_current_jong`에 위임합니다.
     */
     ...
     * `BaseKoreanComposer::set_current_cho`에 위임합니다.
     */
     ...
     * `BaseKoreanComposer::set_current_jung`에 위임합니다.
     */
     ...
     * `BaseKoreanComposer::set_current_jong`에 위임합니다.
     */
     ...
     * `BaseKoreanComposer::get_combined_jamo`에 위임합니다.
     */
     ...
     * `BaseKoreanComposer::is_new_syllable`에 위임합니다.
