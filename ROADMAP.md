...
### 7단계: 입력 방식·플랫폼 확장 (구상 — ①은 v1 완료, 나머지 5개는 미착수)

①은 v1이 이미 구현·완료됐습니다(아래 참고). 나머지 5개는 아직 계획 단계도 아닌 **아이디어 백로그**입니다.
각 항목의 "선행"은 착수 전에 반드시 끝나 있어야 하는 것이고, "난점"은 그 항목이 실패한다면
십중팔구 여기서 실패한다는 지점입니다. 규모는 전부 주 단위 이상이며, 순서는 우선순위가 아닙니다.

- [ ] **① 한자 단어 단위 변환**: **v1 완료** — 단어 최장 접미 변환(방금 친 단어를 한 번에
  `대한민국` → `大韓民國`), 앱에서 마우스로 선택한 한글 단어의 변환, 확정 문자열 서식 설정
  (`漢字`/`한자(漢字)`/`漢字(한자)`)까지 구현됐습니다. 사전(`src/data/hanja.txt`, 다음절 항목 27만
  5천 개)과 `HanjaDictionary` 문자열 키 조회를 그대로 재사용했습니다.
  - 남은 것(v2): 조사가 붙은 `대한민국은` 에서 조사를 자동으로 떼어내는 판정(현재는 조사를 붙이기
    전에 한자 키를 눌러야 함), 확장 키(축소는 v1에서 팝업이 뜬 채 한자 키를 다시 누르면 이미 동작함),
    사전 등장 순이 아닌 후보 랭킹.
